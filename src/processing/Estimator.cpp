...
Estimator::Estimator()
    ...
    m_pose_graph_optimizer = std::make_unique<PoseGraphOptimizer>();
...
void Estimator::handleLoopClosure(int current_id, int candidate_id, Eigen::Matrix4f relative_pose) {
    ...
    spdlog::info("🎯 [LOOP_CLOSURE] Loop closure detected: Frame {} -> Frame {}", 
    ...
    if (m_pgo_enabled_ && m_pose_graph_optimizer) {
        ...
        spdlog::info("[PGO] Loop closure edge added to pose graph: {} -> {}", candidate_id, current_id);
    ...
    {
        spdlog::warn("[PGO] Pose graph optimization disabled or not initialized");
    }
}
...
Eigen::Vector3f Estimator::computeRobustMean(const std::vector<Eigen::Vector3f>& points) {
    ...
    // Simple mean for now - you can enhance this with median or outlier rejection
...
void Estimator::logPGOCorrectionStatistics() {
    if (!Config::getInstance().m_enable_debug_output) return;

    // Calculate overall trajectory correction
    if (m_keyframes.size() >= 2) {
        Eigen::Vector3f first_pose_before, first_pose_after;
        Eigen::Vector3f last_pose_before, last_pose_after;
        
        // This would require storing original poses before PGO
        // For now, just log the current state
        spdlog::info("[PGO] Current trajectory has {} keyframes", m_keyframes.size());
        
        if (!m_keyframes.empty()) {
            Eigen::Vector3f current_position = m_keyframes.back()->get_Twb().block<3,1>(0,3);
            spdlog::info("[PGO] Current position: ({:.3f}, {:.3f}, {:.3f})",
...
// PGO status methods
bool Estimator::isPGOEnabled() const {
    return m_pgo_enabled_;
}

size_t Estimator::getPGONodeCount() const {
    if (m_pose_graph_optimizer) {
        return m_pose_graph_optimizer->getNodeCount();
    }
    return 0;
}

size_t Estimator::getPGOEdgeCount() const {
    if (m_pose_graph_optimizer) {
        return m_pose_graph_optimizer->getEdgeCount();
    }
    return 0;
}

Eigen::Matrix4f Estimator::getCurrentOptimizedPose() const {
    if (m_pose_graph_optimizer && m_current_frame) {
        return m_pose_graph_optimizer->getOptimizedPose(m_current_frame->get_frame_id());
    }
...
std::shared_ptr<Frame> Estimator::create_rgbd_frame(const cv::Mat& rgb_image, const cv::Mat& depth_map, long long timestamp) {
    ...
    cv::Mat gray_image = rgb_image.clone();  // Clone instead of reference
    cv::Mat rgb_image_clone = rgb_image.clone();  // Clone for RGB storage
    // Get camera parameters from global config
    const auto& global_config = Config::getInstance();
    ...
    frame->set_rgb_image(std::move(rgb_image_clone));
    // Set initial pose and velocity
    if (m_previous_frame) {
        // For non-first frames, start with previous frame pose
        // Actual prediction will be done in process_frame() via predict_state()
...
void lightweight_vio::Estimator::create_keyframe(std::shared_ptr<Frame> frame) {
    ...
    if (m_pgo_enabled_ && m_pose_graph_optimizer) {
        m_pose_graph_optimizer->addKeyframeNode(frame);
        retryPendingLoops();
        spdlog::debug("[PGO] Added keyframe {} to pose graph", frame->get_frame_id());
    }
    ...
    {
        cv::Mat image = frame->get_rgb_image();
        if(image.empty())
        {
            spdlog::info("[LOOP_CLOSURE] Keyframe {} rgb image empty", 
                         frame->get_frame_id());
        }
        else
        {
            m_loop_closure_detector->addKeyframe(frame);
            spdlog::info("[LOOP_CLOSURE] Keyframe {} sent to loop closure detector", 
                            frame->get_frame_id());
        }
...
void Estimator::retryPendingLoops() {
    if (m_pending_loops.empty()) return;
    ...
    m_pending_loops.swap(still_pending);
}
